...
16. **acast · `linux-pwsh`** — acast (v0.4.0) + agg (v1.9.0) produced no valid GIF for this cell in CI; see run logs
17. **termsvg · `linux-pwsh`** — no valid GIF produced in CI
18. **termsvg · `windows-pwsh`** — termsvg's record command imports syscall.SIGWINCH (POSIX-only); the record subcommand does not compile on Windows.
