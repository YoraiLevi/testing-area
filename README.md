...
20. **acast · `linux-pwsh`** — acast (v0.4.0) + agg (v1.9.0) produced no valid GIF for this cell in CI; see run logs
21. **termsvg · `linux-pwsh`** — no valid GIF produced in CI
22. **termsvg · `windows-pwsh`** — termsvg's record command imports syscall.SIGWINCH (POSIX-only); the record subcommand does not compile on Windows.
...
**Partial coverage (working / declared cell-scenarios):** [acast][acast] (16/20), [Terminalizer][terminalizer] (16/20), [termsvg][termsvg] (12/16), [VHS][vhs] (16/20).
