# EVP — recording assets

- **Family:** hybrid (scripted tape + interactive live capture)
- **Modes:** interactive + scripted
- **Formats:** gif · svg · svgz
- **Converter:** none (native encoder)
- **Upstream:** https://github.com/HalFrgrd/evp

Files are named `<cell>-<scenario>.<ext>` within each format directory.

## `gif` — 16 file(s)

![EVP gif sample: linux-bash-help-tour.gif](gif/linux-bash-help-tour.gif)

- [`linux-bash-help-tour.gif`](gif/linux-bash-help-tour.gif)
- [`linux-bash-launch-exit.gif`](gif/linux-bash-launch-exit.gif)
- [`linux-bash-tui-splash.gif`](gif/linux-bash-tui-splash.gif)
- [`linux-bash-typing-demo.gif`](gif/linux-bash-typing-demo.gif)
- [`linux-pwsh-help-tour.gif`](gif/linux-pwsh-help-tour.gif)
- [`linux-pwsh-launch-exit.gif`](gif/linux-pwsh-launch-exit.gif)
- [`linux-pwsh-tui-splash.gif`](gif/linux-pwsh-tui-splash.gif)
- [`linux-pwsh-typing-demo.gif`](gif/linux-pwsh-typing-demo.gif)
- [`linux-zsh-help-tour.gif`](gif/linux-zsh-help-tour.gif)
- [`linux-zsh-launch-exit.gif`](gif/linux-zsh-launch-exit.gif)
- [`linux-zsh-tui-splash.gif`](gif/linux-zsh-tui-splash.gif)
- [`linux-zsh-typing-demo.gif`](gif/linux-zsh-typing-demo.gif)
- [`macos-zsh-help-tour.gif`](gif/macos-zsh-help-tour.gif)
- [`macos-zsh-launch-exit.gif`](gif/macos-zsh-launch-exit.gif)
- [`macos-zsh-tui-splash.gif`](gif/macos-zsh-tui-splash.gif)
- [`macos-zsh-typing-demo.gif`](gif/macos-zsh-typing-demo.gif)

## `svg` — 16 file(s)

![EVP svg sample: linux-bash-help-tour.svg](svg/linux-bash-help-tour.svg)

- [`linux-bash-help-tour.svg`](svg/linux-bash-help-tour.svg)
- [`linux-bash-launch-exit.svg`](svg/linux-bash-launch-exit.svg)
- [`linux-bash-tui-splash.svg`](svg/linux-bash-tui-splash.svg)
- [`linux-bash-typing-demo.svg`](svg/linux-bash-typing-demo.svg)
- [`linux-pwsh-help-tour.svg`](svg/linux-pwsh-help-tour.svg)
- [`linux-pwsh-launch-exit.svg`](svg/linux-pwsh-launch-exit.svg)
- [`linux-pwsh-tui-splash.svg`](svg/linux-pwsh-tui-splash.svg)
- [`linux-pwsh-typing-demo.svg`](svg/linux-pwsh-typing-demo.svg)
- [`linux-zsh-help-tour.svg`](svg/linux-zsh-help-tour.svg)
- [`linux-zsh-launch-exit.svg`](svg/linux-zsh-launch-exit.svg)
- [`linux-zsh-tui-splash.svg`](svg/linux-zsh-tui-splash.svg)
- [`linux-zsh-typing-demo.svg`](svg/linux-zsh-typing-demo.svg)
- [`macos-zsh-help-tour.svg`](svg/macos-zsh-help-tour.svg)
- [`macos-zsh-launch-exit.svg`](svg/macos-zsh-launch-exit.svg)
- [`macos-zsh-tui-splash.svg`](svg/macos-zsh-tui-splash.svg)
- [`macos-zsh-typing-demo.svg`](svg/macos-zsh-typing-demo.svg)

## `svgz` — 16 file(s)

![EVP svgz sample: linux-bash-help-tour.svgz](svgz/linux-bash-help-tour.svgz)

- [`linux-bash-help-tour.svgz`](svgz/linux-bash-help-tour.svgz)
- [`linux-bash-launch-exit.svgz`](svgz/linux-bash-launch-exit.svgz)
- [`linux-bash-tui-splash.svgz`](svgz/linux-bash-tui-splash.svgz)
- [`linux-bash-typing-demo.svgz`](svgz/linux-bash-typing-demo.svgz)
- [`linux-pwsh-help-tour.svgz`](svgz/linux-pwsh-help-tour.svgz)
- [`linux-pwsh-launch-exit.svgz`](svgz/linux-pwsh-launch-exit.svgz)
- [`linux-pwsh-tui-splash.svgz`](svgz/linux-pwsh-tui-splash.svgz)
- [`linux-pwsh-typing-demo.svgz`](svgz/linux-pwsh-typing-demo.svgz)
- [`linux-zsh-help-tour.svgz`](svgz/linux-zsh-help-tour.svgz)
- [`linux-zsh-launch-exit.svgz`](svgz/linux-zsh-launch-exit.svgz)
- [`linux-zsh-tui-splash.svgz`](svgz/linux-zsh-tui-splash.svgz)
- [`linux-zsh-typing-demo.svgz`](svgz/linux-zsh-typing-demo.svgz)
- [`macos-zsh-help-tour.svgz`](svgz/macos-zsh-help-tour.svgz)
- [`macos-zsh-launch-exit.svgz`](svgz/macos-zsh-launch-exit.svgz)
- [`macos-zsh-tui-splash.svgz`](svgz/macos-zsh-tui-splash.svgz)
- [`macos-zsh-typing-demo.svgz`](svgz/macos-zsh-typing-demo.svgz)
