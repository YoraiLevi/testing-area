# Terminalizer — recording assets

- **Family:** hybrid (scripted tape + interactive live capture)
- **Modes:** interactive + scripted
- **Formats:** yml · gif
- **Converter:** none (native encoder)
- **Upstream:** https://github.com/faressoft/terminalizer

Files are named `<cell>-<scenario>.<ext>` within each format directory.

## `yml` — 16 file(s)

![Terminalizer yml sample: linux-bash-help-tour.yml](yml/linux-bash-help-tour.yml)

- [`linux-bash-help-tour.yml`](yml/linux-bash-help-tour.yml)
- [`linux-bash-launch-exit.yml`](yml/linux-bash-launch-exit.yml)
- [`linux-bash-tui-splash.yml`](yml/linux-bash-tui-splash.yml)
- [`linux-bash-typing-demo.yml`](yml/linux-bash-typing-demo.yml)
- [`linux-pwsh-help-tour.yml`](yml/linux-pwsh-help-tour.yml)
- [`linux-pwsh-launch-exit.yml`](yml/linux-pwsh-launch-exit.yml)
- [`linux-pwsh-tui-splash.yml`](yml/linux-pwsh-tui-splash.yml)
- [`linux-pwsh-typing-demo.yml`](yml/linux-pwsh-typing-demo.yml)
- [`linux-zsh-help-tour.yml`](yml/linux-zsh-help-tour.yml)
- [`linux-zsh-launch-exit.yml`](yml/linux-zsh-launch-exit.yml)
- [`linux-zsh-tui-splash.yml`](yml/linux-zsh-tui-splash.yml)
- [`linux-zsh-typing-demo.yml`](yml/linux-zsh-typing-demo.yml)
- [`macos-zsh-help-tour.yml`](yml/macos-zsh-help-tour.yml)
- [`macos-zsh-launch-exit.yml`](yml/macos-zsh-launch-exit.yml)
- [`macos-zsh-tui-splash.yml`](yml/macos-zsh-tui-splash.yml)
- [`macos-zsh-typing-demo.yml`](yml/macos-zsh-typing-demo.yml)

## `gif` — 16 file(s)

![Terminalizer gif sample: linux-bash-help-tour.gif](gif/linux-bash-help-tour.gif)

- [`linux-bash-help-tour.gif`](gif/linux-bash-help-tour.gif)
- [`linux-bash-launch-exit.gif`](gif/linux-bash-launch-exit.gif)
- [`linux-bash-tui-splash.gif`](gif/linux-bash-tui-splash.gif)
- [`linux-bash-typing-demo.gif`](gif/linux-bash-typing-demo.gif)
- [`linux-pwsh-help-tour.gif`](gif/linux-pwsh-help-tour.gif)
- [`linux-pwsh-launch-exit.gif`](gif/linux-pwsh-launch-exit.gif)
- [`linux-pwsh-tui-splash.gif`](gif/linux-pwsh-tui-splash.gif)
- [`linux-pwsh-typing-demo.gif`](gif/linux-pwsh-typing-demo.gif)
- [`linux-zsh-help-tour.gif`](gif/linux-zsh-help-tour.gif)
- [`linux-zsh-launch-exit.gif`](gif/linux-zsh-launch-exit.gif)
- [`linux-zsh-tui-splash.gif`](gif/linux-zsh-tui-splash.gif)
- [`linux-zsh-typing-demo.gif`](gif/linux-zsh-typing-demo.gif)
- [`macos-zsh-help-tour.gif`](gif/macos-zsh-help-tour.gif)
- [`macos-zsh-launch-exit.gif`](gif/macos-zsh-launch-exit.gif)
- [`macos-zsh-tui-splash.gif`](gif/macos-zsh-tui-splash.gif)
- [`macos-zsh-typing-demo.gif`](gif/macos-zsh-typing-demo.gif)
